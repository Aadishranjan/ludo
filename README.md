...
2. Fill `.env` with BotFather token, bot username, public app URL, and internal token.
...
- `/start` -> opens bot home/help
- `/play` -> creates room and sends Telegram WebApp button

### Notes

- Bot calls `POST /api/telegram/room` with `x-internal-token`.
- For local testing in Telegram, set `APP_BASE_URL` to your ngrok HTTPS URL.
- Mobile responsive UI updates were added in `app/public/style/index.css` and `app/public/style/ludo.css`.
